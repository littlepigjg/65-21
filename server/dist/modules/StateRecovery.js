...
exports.stateRecovery = exports.StateRecovery = void 0;
const uuid_1 = require("uuid");
const file_1 = require("../utils/file");
const storage_1 = require("../utils/storage");
class StateRecovery {
    static async scanBothSides(config) {
    ...
    static async scanDirectory(dir, source, ignoredPatterns) {
    ...
    static async analyze(sourceFiles, targetFiles) {
    ...
    static async generateConflictsFromDifferences(sourceFiles, targetFiles) {
        const sourceMap = new Map(sourceFiles.map(f => [f.path, f]));
        const targetMap = new Map(targetFiles.map(f => [f.path, f]));
        const allPaths = new Set([...sourceMap.keys(), ...targetMap.keys()]);
        const conflicts = [];
    ...
    static async persistConflicts(conflicts) {
        for (const conflict of conflicts) {
            await (0, storage_1.addConflict)(conflict);
        }
    }
    static async performSafeRecovery(config, options = {}) {
        const opts = {
...
exports.StateRecovery = StateRecovery;
exports.stateRecovery = new StateRecovery();
